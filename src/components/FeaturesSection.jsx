...
function VideoCard({ src, poster, alt, title, text, iconSrc, isLarge, isReverse, onOpenModal }) {
  return (
    <div className={`video-card${isLarge ? '' : ' cc-two'}`}>
      <div className={`video-card_inner${isReverse ? ' cc-first' : ' cc-six'} u-position-relative u-overflow-hidden`}>
        {isReverse ? (
          <>
            <div className="video-card_content">
              ...
              <div className="button-wrapper">
                <a href={TYPEFORM_URL} onClick={(e) => { e.preventDefault(); onOpenModal(TYPEFORM_URL) }} className="button-primary w-inline-block u-weight-500 u-position-relative u-stacking-context u-overflow-hidden">
                  <span>Open Account</span>
                </a>
              </div>
            </div>
            <video autoPlay loop muted playsInline className="video-element cc-mobile lazy" poster={poster}>
            ...
            <video autoPlay loop muted playsInline className="video-element cc-desktop lazy" poster={poster}>
              <source data-src="https://cdn.prod.website-files.com/68185a5ac5e91731a60d1b18%2F682486ae86deecdc8d59725e_Card_01%20%281%29-transcode.mp4" type="video/mp4" />
              <source data-src="https://cdn.prod.website-files.com/68185a5ac5e91731a60d1b18%2F682486ae86deecdc8d59725e_Card_01%20%281%29-transcode.webm" type="video/webm" />
            </video>
          </>
        ) : (
          <>
            <div className="video-card_content">
              <img src={iconSrc} loading="lazy" alt="" />
              <h3 className={`video-card_title${isLarge ? ' cc-small' : ''} heading-h${isLarge ? '6' : '5'} u-text-color-dark`}>
                {title}
              </h3>
              {!isLarge && <div className="video-card_text cc-two text-xl">{text}</div>}
              {isLarge && (
                <div className="button-wrapper">
                  <a href={TYPEFORM_URL} onClick={(e) => { e.preventDefault(); onOpenModal(TYPEFORM_URL) }} className="button-primary w-inline-block u-weight-500 u-position-relative u-stacking-context u-overflow-hidden">
                    <span>Open Account</span>
  ...
  return (
    <section className="section-features u-spacing-inline u-spacing-block u-position-relative">
      <div className="container cc-1125">
        ...
        <VideoCard
          isLarge
          iconSrc="https://cdn.prod.website-files.com/682e10925e475b2f723fc5bc/688cdcab58a7b9b6498761da_wide-choice-of-BINs-icon.svg"
          title={<><span>Wide choice of&nbsp;&nbsp;</span><span className="u-color-primary">BINs</span></>}
          text="More than 50 reliable BINs minimize the risks associated with payments and ad account blocking."
        ...
        <div className="video-card_grid">
          {SMALL_CARDS.map((card, i) => (
            <VideoCard key={i} iconSrc={card.iconSrc} title={card.title} src={card.src} poster={card.poster} onOpenModal={onOpenModal} />
          ))}
        </div>

        <VideoCard
          isReverse
          iconSrc="https://cdn.prod.website-files.com/682e10925e475b2f723fc5bc/688cabc1905674bbd3f3fa4d_instant-issuance-icon.svg"
          title="Instant Issuance"
          text="Generate cards instantly and run your advertising campaigns without delays or hassles."
          poster="https://cdn.prod.website-files.com/682e10925e475b2f723fc5bc/688c9f449aaf6671f51e848b_video-poster-1.jpg"
          onOpenModal={onOpenModal}
        />
      </div>
    </section>
